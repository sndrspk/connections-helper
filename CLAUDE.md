...
- **Responsive grid**: the puzzle grid is 4 columns on desktop and 3 columns below 560px, so mobile tiles stay wide enough for readable words.
- **Tile sizing**: the base tile font size lives in the `--tile-fs` CSS variable, set on `.puzzle-grid` (20px desktop) and `.lane-tiles` (15px). The mobile media query replaces the grid value with a `clamp()` that tracks the 3-column tile width, so the type shrinks gracefully on narrow phones instead of wrapping. `adaptiveStyle` in `Tile.jsx` scales `--tile-fs` down by word length via `calc()` — it never hardcodes pixel sizes, so both breakpoints share one falloff curve.
- **Uniform rows**: grid tiles have a `min-height` derived from `--tile-fs` that reserves two lines of the smallest adaptive size, so a wrapped word takes the same room as a short one. `grid-auto-rows: 1fr` on `.puzzle-grid` equalises the rows even when content exceeds that, and tiles are flex-centred so the text stays vertically centred in the taller box.
